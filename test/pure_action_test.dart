...
void main() {
  group('PurePriorityQueue', () {
    ...
    setUp(() {
      store = PureStore<List<String>>(
        ...
        batchDelay: const Duration(
          milliseconds: 100,
        ), // Long delay to batch actions up
      );
    });
    ...
    test('executes actions in priority order', () async {
      ...
      store
        ..dispatch(
          ActionWithDelay('slow', const Duration(milliseconds: 50)),
        )
        // Dispatch mixed priority actions immediately after
        ..dispatch(LoggingAction('normal'))
        ..dispatch(LoggingAction('high', priority: 10))
        ..dispatch(LoggingAction('low', priority: -10))
        ..dispatch(LoggingAction('critical', priority: 100));
...
class ActionWithDelay extends PureAction<List<String>> {
  ActionWithDelay(this.name, this.delay, {int priority = 0})
    : _priority = priority;
  @override
  final String name;
