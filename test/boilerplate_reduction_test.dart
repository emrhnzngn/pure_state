  ...
  group('Boilerplate Reduction Tests', () {
    ...
    test('store.mutate should update state correctly', () async {
      final store = PureStore<TestState>(TestState())
        ..mutate((state) => state.copyWith(count: 10), name: 'UpdateCount');
    ...
    test(
      'store.mutate should work consistently with multiple updates',
      () async {
        final store = PureStore<TestState>(TestState());

        store
          ..mutate(
            (state) => state.copyWith(count: store.state.count + 1),
            name: 'Inc1',
          )
          ..mutate(
            (state) => state.copyWith(count: store.state.count + 1),
    ...
    test('NamedAction name is preserved in Middleware', () async {
      ...
      store
        ...
        ..mutate((state) => state, name: 'DebugMe');
